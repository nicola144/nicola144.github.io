...
Clearly, if we want this quantity to be small, then whenever the numerator is high , then $$q$$ should also be at least as high. That is, when .. is high, then $$q(\mathbf{x})$$ should be high, or at least it should definitely not be small. For a good approximation, we need  ... high   $$\Rightarrow q(\mathbf{x})$$ large. In fact, the proposal that minimizes the variance turns out to be a normalized version of .. : 
