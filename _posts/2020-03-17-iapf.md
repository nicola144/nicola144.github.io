...
Setting the approximation to the predictive likelihood (the true is equal to $$\int {\color{blue}{f}}(x_{t+1} \mid x_{t}) \cdot {\color{green}{g}}(y_{t+1} \mid x_{t+1}) \mathrm{d} x_{t+1} $$) to $$\hat{p}(y_{t} \mid x_{t-1}) = g(y_t \mid \mu(x_{t-1})) $$ where $$ \mu(x_{t-1})$$ is some likely value is common. 
