...
This is where Sequential Monte Carlo or Sequential Importance Resampling or particle filtering algorithms come into the picture. They mitigate the weight degeneracy issue by changing the particle set that was found in the previous iterationl. They do so by resampling with replacement an equally sized particle set, where each sample is sampled with probability equal to its weight. 
