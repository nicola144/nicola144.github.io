...
1. **Propagation** : sample from proposal $$\mathbf{s}_{1}^{n} \sim q_1(\mathbf{s}_1)$$
2. **Update**: compute weights $$w_{1}^{n} \propto \frac{p(\mathbf{s}_{1}^{n}, \mathbf{v}_{1}^{n})}{q_1(\mathbf{s}_{1}^{n})}$$
3. **Resample**: $$\left \{ \mathbf{s}_{1}^{n} , w_{1}^{n} \right \}_{n=1}^{N} $$ to obtain $$ \left \{ \mathbf{r}_{1}^{n}, 1/N \right \}_{n=1}^{N} $$

At time $$t \geq 2$$:

1. **Propagation** : sample from proposal $$\mathbf{s}_{t}^{n} \sim q_t(\mathbf{s}_{t} \mid \mathbf{r}_{1:t-1}^{n})$$ and set $$ \mathbf{s}_{1:t}^{n} \leftarrow (\mathbf{r}_{1:t-1}^{n}, \mathbf{s}_{t}^{n})$$
...
$$\begin{equation}\begin{aligned}
\tilde{w} &= \tilde{w}_{t-1}(\mathbf{s}_{1:t-1}) \cdot \frac{\gamma_{t}(\mathbf{s}_{1:t})}{\gamma_{t-1}(\mathbf{s}_{1:t-1}) q_{t}(\mathbf{s}_{t}\mid \mathbf{s}_{1:t-1})} \\
&=  \tilde{w}_{t-1}(\mathbf{s}_{1:t-1}) \cdot  \frac{p(\mathbf{s}_{1:t}, \mathbf{v}_{1:t}) p(\mathbf{v}_{t+1} \mid \mathbf{s}_{t})}{p(\mathbf{s}_{1:t-1}, \mathbf{v}_{1:t-1}) p(\mathbf{v}_{t} \mid \mathbf{s}_{t-1}) q(\mathbf{s}_{t} \mid \mathbf{s}_{1:t-1})} \\
&= W_{t-1}(\mathbf{s}_{1:t-1}) \cdot \frac{\cancel{p(\mathbf{s}_{1:t-1}, \mathbf{v}_{1:t-1})} \color{blue}{f}(\mathbf{s}_{t} \mid \mathbf{s}_{t-1}) \color{green}{g}(\mathbf{v}_{t} \mid \mathbf{s}_{t}) p(\mathbf{v}_{t+1} \mid \mathbf{s}_{t})}{\cancel{p(\mathbf{s}_{1:t-1}, \mathbf{v}_{1:t-1})} p(\mathbf{v}_{t} \mid \mathbf{s}_{t-1}) q(\mathbf{s}_{t} \mid \mathbf{s}_{1:t-1})} 
...
## The Improved Auxiliary Particle Filter through the Multiple Importance Sampling Interpretation <a name="paragraph3"></a>
