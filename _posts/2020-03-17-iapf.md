...
2. **Delayed resampling step:** Sample with replacement from the previous particle set with probabilities $$\lambda_{t}^{m}$$ to obtain $$\left \{ \mathbf{r}_{t-1}^{m} \right \}_{m=1}^{M} $$ as well as associated means $$\left \{  \boldsymbol{\mu}_{t_{r}}^{m} \right \} $$
