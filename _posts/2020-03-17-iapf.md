...
$$\begin{equation}\begin{aligned}
 ...
 &= W_{t-1}(\mathbf{s}_{1:t-1}) \cdot \frac{\gamma_{t}(\mathbf{s}_{1:t})}{\gamma_{t-1}(\mathbf{s}_{1:t-1}) q_{t}(x_{t}\mid \mathbf{s}_{1:t-1})}
...
so that the trick of expressing the new weight in \eqref{eq13} in terms of the previous weights is possible. Hence, the only choice goes into $$q_{t}\left(x_{t} \mid x_{1: t-1}\right)$$. When the target is $$\gamma_{t}(\mathbf{s}_{1:t}) = p(\mathbf{s}_{1:t}, \mathbf{v}_{1:t}) = p(\mathbf{s}_{1:t-1}, \mathbf{v}_{1:t-1}) \color{blue}{f}(\mathbf{s}_{t} \mid \mathbf{s}_{t-1}) \cdot \color{green}{g}(\mathbf{v}_{t} \mid \mathbf{s}_{t})$$ , then the weight update in \eqref{eq13} becomes : 


$$\begin{equation}\begin{aligned}
&= W_{t-1}(\mathbf{s}_{1:t-1}) \cdot \frac{\gamma_{t}(\mathbf{s}_{1:t})}{\gamma_{t-1}(\mathbf{s}_{1:t-1}) q_{t}(x_{t}\mid \mathbf{s}_{1:t-1})} \\
&=  W_{t-1}(\mathbf{s}_{1:t-1}) \cdot \frac{\color{blue}{f}(x_{t}\mid \mathbf{s}_{t-1}) \color{green}{g}(\mathbf{v}_{t} \mid \mathbf{s}_{t}) \overbrace{p(\mathbf{s}_{1:t-1}, \mathbf{v}_{1:t-1})}^{\cancel{\gamma_{t-1}(\mathbf{s}_{1:t-1})}}}{\cancel{\gamma_{t-1}(\mathbf{s}_{1:t-1})} q(\mathbf{s}_{t} \mid \mathbf{s}_{1:t-1})} \\
&= W_{t-1}(\mathbf{s}_{1:t-1}) \cdot \frac{\color{blue}{f}(x_{t}\mid \mathbf{s}_{t-1}) \color{green}{g}(\mathbf{v}_{t} \mid \mathbf{s}_{t})}{q(\mathbf{s}_{t} \mid \mathbf{s}_{1:t-1})}
...
$$\begin{equation}\begin{aligned}
\gamma_{t}(\mathbf{s}_{1:t}) := p(\mathbf{s}_{1:t} , y_{\color{red}{t+1}}) &= \int p(x_{1:t+1} , y_{1:t+1}) \mathrm{d} \mathbf{s}_{t+1} \\
&= \int p(\mathbf{s}_{1:t} , \mathbf{v}_{1:t}) \cdot {\color{blue}{f}}(\mathbf{s}_{t+1} \mid x_{t}) \cdot {\color{green}{g}}(\mathbf{v}_{t+1} \mid \mathbf{s}_{t+1}) \mathrm{d} \mathbf{s}_{t+1} \\
&=  p(\mathbf{s}_{1:t} , \mathbf{v}_{1:t}) \int {\color{blue}{f}}(\mathbf{s}_{t+1} \mid x_{t}) \cdot {\color{green}{g}}(\mathbf{v}_{t+1} \mid \mathbf{s}_{t+1}) \mathrm{d} \mathbf{s}_{t+1} \\
&=  p(\mathbf{s}_{1:t} , \mathbf{v}_{1:t}) \cdot \underbrace{p(\mathbf{v}_{t+1} \mid x_{t})}_{"predictive~likelihood"}
...
&= W_{t-1}(\mathbf{s}_{1:t-1}) \cdot \frac{\cancel{p(\mathbf{s}_{1:t-1}, \mathbf{v}_{1:t-1})} \color{blue}{f}(x_{t} \mid \mathbf{s}_{t-1}) \color{green}{g}(\mathbf{v}_{t} \mid \mathbf{s}_{t}) \hat{p}(\mathbf{v}_{t+1} \mid \mathbf{s}_{t})}{\cancel{p(\mathbf{s}_{1:t-1}, \mathbf{v}_{1:t-1})} \hat{p}(\mathbf{v}_{t} \mid \mathbf{s}_{t-1}) q(\mathbf{s}_{t} \mid \mathbf{s}_{1:t-1})} 
\end{aligned}\end{equation}\tag{17}\label{eq17}$$

since we have a different target $$ \gamma_{t}(\mathbf{s}_{1:t})$$ to the unnormalized filtering $$p(\mathbf{s}_{1:t}, \mathbf{v}_{1:t})$$, the algorithm actually sequentially estimates $$\left \{ \hat{p}(\mathbf{s}_{1:t} \mid y_{1:t+1}) = p(\mathbf{s}_{1:t} \mid \mathbf{v}_{1:t}) \hat{p}(\mathbf{v}_{t+1} \mid \mathbf{s}_{t}) \right \}_{t \geq 1} $$ , therefore we need to use importance sampling to obtain $$\left \{ p(\mathbf{s}_{1:t} \mid \mathbf{v}_{1:t}) \propto p(\mathbf{s}_{1:t}, \mathbf{v}_{1:t}) \right \}_{t \geq 1}$$, with the following importance distribution:

$$\begin{equation}\begin{aligned}
\hat{p}(\mathbf{s}_{1:t-1} , \mathbf{v}_{1:t}) q_{t}(x_{t} \mid \mathbf{v}_{t}, \mathbf{s}_{t-1}) = p(\mathbf{s}_{1:t-1}, \mathbf{v}_{1:t-1}) \hat{p}(\mathbf{v}_{t} \mid \mathbf{s}_{t-1}) q_{t}(x_{t} \mid \mathbf{v}_{t}, \mathbf{s}_{t-1})
\end{aligned}\end{equation}\tag{18}\label{eq18}$$

So that the weight update becomes (what we actually want divided by proposal):

$$\begin{equation}\begin{aligned}
&= W_{t-1}(\mathbf{s}_{1:t-1}) \cdot  \frac{p(\mathbf{s}_{1:t}, \mathbf{v}_{1:t})}{\hat{p}(\mathbf{s}_{1:t-1} , \mathbf{v}_{1:t}) q_{t}(x_{t} \mid \mathbf{v}_{t}, \mathbf{s}_{t-1})} \\
&= W_{t-1}(\mathbf{s}_{1:t-1}) \cdot  \frac{p(\mathbf{s}_{1:t}, \mathbf{v}_{1:t})}{ p(\mathbf{s}_{1:t-1}, \mathbf{v}_{1:t-1}) \hat{p}(\mathbf{v}_{t} \mid \mathbf{s}_{t-1}) q_{t}(x_{t} \mid \mathbf{v}_{t}, \mathbf{s}_{t-1})}  \\
&=  W_{t-1}(\mathbf{s}_{1:t-1}) \cdot \frac{\cancel{p(\mathbf{s}_{1:t-1}, \mathbf{v}_{1:t-1})}f(x_{t}\mid \mathbf{s}_{t-1}) g(\mathbf{v}_{t} \mid \mathbf{s}_{t})}{\cancel{p(\mathbf{s}_{1:t-1}, \mathbf{v}_{1:t-1})}\hat{p}(\mathbf{v}_{t} \mid \mathbf{s}_{t-1}) q_{t}(\mathbf{s}_{t} \mid \mathbf{v}_{t}, \mathbf{s}_{t-1})} \\
&= W_{t-1}(\mathbf{s}_{1:t-1}) \cdot \frac{f(x_{t}\mid \mathbf{s}_{t-1}) g(\mathbf{v}_{t} \mid \mathbf{s}_{t})}{\hat{p}(\mathbf{v}_{t} \mid \mathbf{s}_{t-1}) q_{t}(\mathbf{s}_{t} \mid \mathbf{v}_{t}, \mathbf{s}_{t-1})}
...
Setting the approximation to the predictive likelihood (the true is equal to $$ p(\mathbf{v}_{t} \mid \mathbf{s}_{t-1}) = \int {\color{blue}{f}}(x_{t} \mid \mathbf{s}_{t-1}) \cdot {\color{green}{g}}(\mathbf{v}_{t} \mid x_{t}) \mathrm{d} x_{t} $$) to $$\hat{p}(\mathbf{v}_{t} \mid \mathbf{s}_{t-1}) = g(\mathbf{v}_{t} \mid \mu(\mathbf{s}_{t-1})) $$ where $$ \mu(\mathbf{s}_{t-1})$$ is some likely value is common. For example , if we choose as approximation to the predictive likelihood : $$\hat{p}(\mathbf{v}_{t} \mid \mathbf{s}_{t-1}) = g(\mathbf{v}_{t} \mid \mu_{t-1}) $$  where $$\mu_{t-1}$$ is the mean of $$ f(\mathbf{s}_{t} \mid \mathbf{s}_{t-1}) $$ *and* we also choose $$ q_t(x_{t} \mid \mathbf{v}_{t}, \mathbf{s}_{t-1}) = f(\mathbf{s}_{t} \mid \mathbf{s}_{t-1})$$ , then we recover as special case the popular version of the APF weights: 

$$\begin{equation}\begin{aligned}
&= W_{t-1}(\mathbf{s}_{1:t-1}) \cdot \frac{f(x_{t}\mid \mathbf{s}_{t-1}) g(\mathbf{v}_{t} \mid \mathbf{s}_{t})}{\hat{p}(\mathbf{v}_{t} \mid \mathbf{s}_{t-1}) q_{t}(\mathbf{s}_{t} \mid \mathbf{v}_{t}, \mathbf{s}_{t-1})} \\
&= W_{t-1}(\mathbf{s}_{1:t-1}) \cdot \frac{\cancel{f(x_{t}\mid \mathbf{s}_{t-1})} g(\mathbf{v}_{t} \mid \mathbf{s}_{t})}{g(\mathbf{v}_{t} \mid \mu_{t-1}) \cancel{f(x_{t}\mid \mathbf{s}_{t-1})}}
