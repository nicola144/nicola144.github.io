...
so that the trick of expressing the new weight in  in terms of the previous weights is possible. Hence, the only choice goes into $$q_{t}\left(x_{t} | x_{1: t-1}\right)$$. When the target is $$\gamma_{t}(x_{1:t}) = p(x_{1:t}, y_{1:t})$$ , then the weight update in  becomes : 
