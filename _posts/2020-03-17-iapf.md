...
$$\begin{equation}\begin{aligned}
 W_{n}\left(\mathbf{s}_{1:t}\right) &=\frac{\gamma_{t}\left(\mathbf{s}_{1:t}\right)}{q_{t}\left(\mathbf{s}_{1:t}\right)} \\ &=\frac{1}{q_{t-1}\left(x_{1: t-1}\right)} \frac{\gamma_{t-1}\left(x_{1: t-1}\right)}{\gamma_{t-1}\left(x_{1: t-1}\right)} \frac{\gamma_{t}\left(\mathbf{s}_{1:t}\right)}{q_{t}\left(\mathbf{s}_{t} | x_{1: t-1}\right)} \\ &=\frac{\gamma_{t-1}\left(x_{1: t-1}\right)}{q_{t-1}\left(x_{1: t-1}\right)} \frac{\gamma_{t}\left(\mathbf{s}_{1:t}\right)}{\gamma_{t-1}\left(x_{1: t-1}\right) q_{t}\left(\mathbf{s}_{t} | x_{1: t-1}\right)} \\
...
$$\begin{equation}\begin{aligned}
q_{t}\left(\mathbf{s}_{1:t}\right)=q_{t-1}\left(x_{1: t-1}\right) q_{t}\left(\mathbf{s}_{t} | x_{1: t-1}\right)
\end{aligned}\end{equation}\tag{14}\label{eq14}$$

so that the trick of expressing the new weight in \eqref{eq13} in terms of the previous weights is possible. Hence, the only choice goes into $$q_{t}\left(\mathbf{s}_{t} \mid x_{1: t-1}\right)$$. When the target is $$\gamma_{t}(\mathbf{s}_{1:t}) = p(\mathbf{s}_{1:t}, \mathbf{v}_{1:t}) = p(\mathbf{s}_{1:t-1}, \mathbf{v}_{1:t-1}) \color{blue}{f}(\mathbf{s}_{t} \mid \mathbf{s}_{t-1}) \cdot \color{green}{g}(\mathbf{v}_{t} \mid \mathbf{s}_{t})$$ , then the weight update in \eqref{eq13} becomes : 
...
In the APF we want to make use of $$ \mathbf{v}_{t+1}$$ and thus set the target $$\gamma_{t}(\mathbf{s}_{1:t}) = p(\mathbf{s}_{1:t}, y_{1:\color{red}{t+1}})$$. By expanding this we can express it as:

$$\begin{equation}\begin{aligned}
\gamma_{t}(\mathbf{s}_{1:t}) := p(\mathbf{s}_{1:t} , y_{\color{red}{t+1}}) &= \int p(\mathbf{s}_{1:t+1} , \mathbf{v}_{1:t+1}) \mathrm{d} \mathbf{s}_{t+1} \\
