...
so that the trick of expressing the new weight in \eqref{eq14} in terms of the previous weights is possible. Hence, the only choice goes into $$q_{t}\left(\mathbf{s}_{t} \mid \mathbf{s}_{1:t-1}\right)$$. When the target is $$\gamma_{t}(\mathbf{s}_{1:t}) = p(\mathbf{s}_{1:t}, \mathbf{v}_{1:t}) = p(\mathbf{s}_{1:t-1}, \mathbf{v}_{1:t-1}) \color{blue}{f}(\mathbf{s}_{t} \mid \mathbf{s}_{t-1}) \cdot \color{green}{g}(\mathbf{v}_{t} \mid \mathbf{s}_{t})$$ , then the weight update in \eqref{eq14} becomes : 
