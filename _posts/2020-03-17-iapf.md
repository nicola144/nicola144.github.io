...
Given this, you should be able to see that we are able to compute \eqref{eq5} sequentially in constant time per step. As we will see, particle filtering methods approximate a distribution with a (weighted) set of samples. Then, if we have samples from the joint $$p(\mathbf{s}_{1:t}, \mathbf{v}_{1:t} )$$, to obtain samples from the unnormalized filtering  distribution $$p(\mathbf{s}_{t}, \mathbf{v}_{1:t} )$$ we can simply discard samples for states $$ \mathbf{s}_{1:t-1}$$, because $$p(\mathbf{s}_{t}, \mathbf{v}_{1:t} )$$ is a marginal of $$p(\mathbf{s}_{t}, \mathbf{v}_{1:t} )$$.
