...
since we have a different target $$ \gamma_{t}(x:_{1:t})$$ to the unnormalized filtering $$p(x_{1:t}, y_{1:t})$$, the algorithm actually sequentially estimates $$\left \{ \hat{p}(x_{1:t} \mid y_{1:t+1}) = p(x_{1:t} \mid y_{1:t}) \hat{p}(y_{t+1} \mid x_t) \right \}_{t \geq 1} $$ , therefore we need to use importance sampling to obtain $$\left \{ p(x_{1:t} \mid y_{1:t}) \right \}_{t \geq 1}$$, with the following importance distribution:

$$\begin{equation}\begin{aligned}
\hat{p}(x_{1:t-1} \mid y_{1:t}) q_{t}(x_{t} \mid y_t, x_{t-1})
...
&= W_{t-1}(x_{1:t-1}) \cdot  \frac{\frac{p(x_{1:t}, y_{1:t})}{Z}}{\hat{p}(x_{1:t-1} \mid y_{1:t}) q_{t}(x){t} \mid y_t, x_{t-1})} \\
...
Setting the approximation to the predictive likelihood (the true is equal to $$ p(y_t \mid x_{t-1}) = \int {\color{blue}{f}}(x_{t} \mid x_{t-1}) \cdot {\color{green}{g}}(y_{t} \mid x_{t}) \mathrm{d} x_{t} $$) to $$\hat{p}(y_{t} \mid x_{t-1}) = g(y_t \mid \mu(x_{t-1})) $$ where $$ \mu(x_{t-1})$$ is some likely value is common. 
