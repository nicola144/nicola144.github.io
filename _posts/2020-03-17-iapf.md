...
This is how we motivate wanting to use $$\mathbf{v}_{t+1}$$ in APF. It is shown that APF can be reinterpreted as a standard SMC algorithm that uses a different "target" distribution. I'll explain what is meant by "target". 
In standard filtering , we use the following recursion to update the weights: 

$$\begin{equation}\begin{aligned}
 W_{t}\left(\mathbf{s}_{1:t}\right) &=\frac{\gamma_{t}\left(\mathbf{s}_{1:t}\right)}{q_{t}\left(\mathbf{s}_{1:t}\right)} \\ &=\frac{1}{q_{t-1}\left(\mathbf{s}_{1:t-1}\right)} \frac{\gamma_{t-1}\left(\mathbf{s}_{1:t-1}\right)}{\gamma_{t-1}\left(\mathbf{s}_{1:t-1}\right)} \frac{\gamma_{t}\left(\mathbf{s}_{1:t}\right)}{q_{t}\left(\mathbf{s}_{t} | \mathbf{s}_{1:t-1}\right)} \\ &=\frac{\gamma_{t-1}\left(\mathbf{s}_{1:t-1}\right)}{q_{t-1}\left(\mathbf{s}_{1:t-1}\right)} \frac{\gamma_{t}\left(\mathbf{s}_{1:t}\right)}{\gamma_{t-1}\left(\mathbf{s}_{1:t-1}\right) q_{t}\left(\mathbf{s}_{t} | \mathbf{s}_{1:t-1}\right)} \\
...
In SMC for filtering i.e. the sequential estimation of $$\left \{ p( \mathbf{s}_{1:t} \mid \mathbf{v}_{1:t}) \right \}_{t>=1} $$, the "target" $$\gamma_{t}(\mathbf{s}_{1:t}) :=  p(\mathbf{s}_{1:t}, \mathbf{v}_{1:t})$$ i.e. the unnormalized posterior. We are also making use of a proposal with the structure : 

$$\begin{equation}\begin{aligned}
q_{t}\left(\mathbf{s}_{1:t}\right)=q_{t-1}\left(\mathbf{s}_{1:t-1}\right) q_{t}\left(\mathbf{s}_{t} | \mathbf{s}_{1:t-1}\right)
\end{aligned}\end{equation}\tag{17}\label{eq17}$$

so that the trick of expressing the new weight in \eqref{eq16} in terms of the previous weights is possible. Hence, the only choice goes into $$q_{t}\left(\mathbf{s}_{t} \mid \mathbf{s}_{1:t-1}\right)$$. When the target is $$\gamma_{t}(\mathbf{s}_{1:t}) = p(\mathbf{s}_{1:t}, \mathbf{v}_{1:t}) = p(\mathbf{s}_{1:t-1}, \mathbf{v}_{1:t-1}) \color{blue}{f}(\mathbf{s}_{t} \mid \mathbf{s}_{t-1}) \cdot \color{green}{g}(\mathbf{v}_{t} \mid \mathbf{s}_{t})$$ , then the weight update in \eqref{eq16} becomes : 


$$\begin{equation}\begin{aligned}
&= W_{t-1}(\mathbf{s}_{1:t-1}) \cdot \frac{\gamma_{t}(\mathbf{s}_{1:t})}{\gamma_{t-1}(\mathbf{s}_{1:t-1}) q_{t}(\mathbf{s}_{t}\mid \mathbf{s}_{1:t-1})} \\
&=  W_{t-1}(\mathbf{s}_{1:t-1}) \cdot \frac{\color{blue}{f}(\mathbf{s}_{t}\mid \mathbf{s}_{t-1}) \color{green}{g}(\mathbf{v}_{t} \mid \mathbf{s}_{t}) \overbrace{p(\mathbf{s}_{1:t-1}, \mathbf{v}_{1:t-1})}^{\cancel{\gamma_{t-1}(\mathbf{s}_{1:t-1})}}}{\cancel{\gamma_{t-1}(\mathbf{s}_{1:t-1})} q(\mathbf{s}_{t} \mid \mathbf{s}_{1:t-1})} \\
&= W_{t-1}(\mathbf{s}_{1:t-1}) \cdot \frac{\color{blue}{f}(\mathbf{s}_{t}\mid \mathbf{s}_{t-1}) \color{green}{g}(\mathbf{v}_{t} \mid \mathbf{s}_{t})}{q(\mathbf{s}_{t} \mid \mathbf{s}_{1:t-1})}
...
\end{aligned}\end{equation}\tag{18}\label{eq19}$$
