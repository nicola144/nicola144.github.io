...
$$\begin{equation}\begin{aligned}
\widehat{\mathcal{I}_{SN}} := \mathbb{E}_{\pi} \left [ f(\mathbf{x}) \right ] &= \int f(\mathbf{x}) \pi(\mathbf{x}) \mathrm{d} \mathbf{x} \\
...
In other words, to obtain a sample from the full proposal at time $$t$$, we can just take the previous trajectory that was sampled up to $$t-1$$ and append a sample from $$ \color{#FF8000}{q}_{t}\left(\mathbf{s}_{t} | \mathbf{s}_{1:t-1}\right)$$. We can now exploit this proposal structure to express the weights at time $$t$$ as a product between the previous weights at $$t-1$$ with a factor. The algebraic trick uses multiplying and dividing by the target at $$t-1$$ to artificially obtain the weights at $$t-1$$:
...
Moreover, for the APF it assumed the common approximation to the predictive likelihood described earlier $$g(\mathbf{v}_{t} \mid \boldsymbol{\mu}_{t-1})$$, where $$ \boldsymbol{\mu}_{t-1} := \mathbb{E}_{\color{blue}{f}(\mathbf{s}_{t} \mid \mathbf{s}_{t-1})} \left \[ \mathbf{s}_t \right \] $$. 
