...
$$
\color{blue}{\text{transition}}: \qquad \mathbf{s}_{t} \sim \color{blue}{f}(\mathbf{s}_{t} \mid \mathbf{s}_{t-1}) \tag{1}\label{eq1}
$$

Further, usually the observation or visible is sampled according to the current state:

$$
\mathbf{v}_{t} \sim \color{green}{g}(\mathbf{v}_{t} \mid \mathbf{s}_{t}) \tag{1}\label{eq2}
$$
