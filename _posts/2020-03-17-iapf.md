...
$$
p(\mathbf{x}) \approx \frac{1}{N}\sum_{n=1}^{N} p(\mathbf{x}_n) \delta_{\mathbf{x}}(\mathbf{x}_{n})
$$
...
Let's return in the context of Bayesian inference, where we have a target posterior distribution $$ \pi(\mathbf{x}) = p(\mathbf{x} \mid \mathcal{D}) $$ where $$ \mathcal{D}$$ is any observed data. For example, in state space models $$\mathcal{D} = p(\mathbf{v}_{1:t})$$. Consider an integral of some function of $$ \mathbf{x}$$ under the posterior: 
...
so that the trick of expressing the new weight in \eqref{eq13} in terms of the previous weights is possible. Hence, the only choice goes into $$q_{t}\left(\mathbf{s}_{t} \mid \mathbf{s}_{1:t-1}\right)$$. When the target is $$\gamma_{t}(\mathbf{s}_{1:t}) = p(\mathbf{s}_{1:t}, \mathbf{v}_{1:t}) = p(\mathbf{s}_{1:t-1}, \mathbf{v}_{1:t-1}) \color{blue}{f}(\mathbf{s}_{t} \mid \mathbf{s}_{t-1}) \cdot \color{green}{g}(\mathbf{v}_{t} \mid \mathbf{s}_{t})$$ , then the weight update in \eqref{eq13} becomes : 
