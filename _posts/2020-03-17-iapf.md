...
$$\begin{equation}\begin{aligned}
 W_{n}\left(\mathbf{s}_{1:t}\right) &=\frac{\gamma_{t}\left(\mathbf{s}_{1:t}\right)}{q_{t}\left(\mathbf{s}_{1:t}\right)} \\ &=\frac{1}{q_{t-1}\left(\mathbf{s}_{1:t-1}\right)} \frac{\gamma_{t-1}\left(\mathbf{s}_{1:t-1}\right)}{\gamma_{t-1}\left(\mathbf{s}_{1:t-1}\right)} \frac{\gamma_{t}\left(\mathbf{s}_{1:t}\right)}{q_{t}\left(\mathbf{s}_{t} | \mathbf{s}_{1:t-1}\right)} \\ &=\frac{\gamma_{t-1}\left(\mathbf{s}_{1:t-1}\right)}{q_{t-1}\left(\mathbf{s}_{1:t-1}\right)} \frac{\gamma_{t}\left(\mathbf{s}_{1:t}\right)}{\gamma_{t-1}\left(\mathbf{s}_{1:t-1}\right) q_{t}\left(\mathbf{s}_{t} | \mathbf{s}_{1:t-1}\right)} \\
...
Setting the approximation to the predictive likelihood (the true is equal to $$ p(\mathbf{v}_{t} \mid \mathbf{s}_{t-1}) = \int {\color{blue}{f}}(\mathbf{s}_{t} \mid \mathbf{s}_{t-1}) \cdot {\color{green}{g}}(\mathbf{v}_{t} \mid \mathbf{s}_{t}) \mathrm{d} \mathbf{s}_{t} $$) to $$\hat{p}(\mathbf{v}_{t} \mid \mathbf{s}_{t-1}) = g(\mathbf{v}_{t} \mid \mu(\mathbf{s}_{t-1})) $$ where $$ \mu(\mathbf{s}_{t-1})$$ is some likely value is common. For example , if we choose as approximation to the predictive likelihood : $$\hat{p}(\mathbf{v}_{t} \mid \mathbf{s}_{t-1}) = g(\mathbf{v}_{t} \mid \mu_{t-1}) $$  where $$\mu_{t-1}$$ is the mean of $$ f(\mathbf{s}_{t} \mid \mathbf{s}_{t-1}) $$ *and* we also choose $$ q_t(\mathbf{s}_{t} \mid \mathbf{v}_{t}, \mathbf{s}_{t-1}) = f(\mathbf{s}_{t} \mid \mathbf{s}_{t-1})$$ , then we recover as special case the popular version of the APF weights: 

$$\begin{equation}\begin{aligned}
&= W_{t-1}(\mathbf{s}_{1:t-1}) \cdot \frac{f(\mathbf{s}_{t}\mid \mathbf{s}_{t-1}) g(\mathbf{v}_{t} \mid \mathbf{s}_{t})}{\hat{p}(\mathbf{v}_{t} \mid \mathbf{s}_{t-1}) q_{t}(\mathbf{s}_{t} \mid \mathbf{v}_{t}, \mathbf{s}_{t-1})} \\
&= W_{t-1}(\mathbf{s}_{1:t-1}) \cdot \frac{\cancel{f(\mathbf{s}_{t}\mid \mathbf{s}_{t-1})} g(\mathbf{v}_{t} \mid \mathbf{s}_{t})}{g(\mathbf{v}_{t} \mid \mu_{t-1}) \cancel{f(\mathbf{s}_{t}\mid \mathbf{s}_{t-1})}}
