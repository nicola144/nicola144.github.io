...
2. **Delayed resampling step:** Sample with replacement from the previous particle set with probabilities $$\lambda_{t}^{m}$$ to obtain $$\left \{ \mathbf{r}_{t-1}^{m} \right \}_{m=1}^{M} $$ as well as associated means $$\left \{  ^{r \hspace{-1pt}}\boldsymbol{\mu}_{t}^{m} \right \}_{m=1}^{M} $$. Here however, instead of considering this generic resampling with a new particle set, let's be more specific. Notice that if this step uses multinomial resampling, what we just said is equivalent to: 
