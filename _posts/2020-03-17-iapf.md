...
&= \tilde{w}_{t-1}(\mathbf{s}_{1:t-1}) \cdot \frac{\cancel{f(\mathbf{s}_{t}\mid \mathbf{s}_{t-1})} \color{green}{g}(\mathbf{v}_{t} \mid \mathbf{s}_{t})}{\color{green}{g}(\mathbf{v}_{t} \mid \boldsymbol{\mu}_{t-1}) \cancel{f(\mathbf{s}_{t}\mid \mathbf{s}_{t-1})}}
...
Moreover, for the APF it assumed the common approximation to the predictive likelihood described earlier $$g(\mathbf{v}_{t} \mid \boldsymbol{\mu}_{t-1})$$, where $$ \boldsymbol{\mu}_{t-1} := \mathbb{E}_{\color{blue}{f}(\mathbf{s}_{t} \mid \mathbf{s}_{t-1})} [ \mathbf{s}_t ] $$. Finally, the proposal is selected to be the transition density. 

---

**Algorithm 3: APF (again)**

At time $$t=1$$: draw M i.i.d. samples from the "proposal" $$ \color{blue}{f}(\mathbf{s}_1) $$

At time $$t \geq 2$$, with particle/weight set $$\left \{ \mathbf{s}_{t-1}^{m}, w_{t-1}^{m} \right \}_{m=1}^{M} $$:

1. **Preweights computation: 
    - Compute $$\boldsymbol{\mu}_{t-1}^{m} := \mathbb{E}_{\color{blue}{f}(\mathbf{s}_{t} \mid \mathbf{s}_{t-1}^{m})} [ \mathbf{s}_t ]$$  for all $$m$$
    - *Preweights* are computed as:
    $$ \lambda_{t}^{m} \propto \color{green}{g}(\mathbf{v}_{t} \mid \boldsymbol{\mu}_{t-1}^{m}) w_{t-1}^{m} $$ for all $$m$$
2. **Delayed resampling step:** Sample with replacement from the previous particle set with probabilities $$\lambda_{t}^{m}$$ to obtain $$\left \{ \mathbf{r}_{t-1}^{m} \right \}_{m=1}^{M} $$ as well as associated means $$\left \{  ^{r \hspace{-1pt}}\boldsymbol{\mu}_{t}^{m} \right \} $$
    
3. **Propagation**: Sample $$\mathbf{s}_{t}^{m} \sim \color{blue}{f}(\mathbf{s}_t \mid \mathbf{r}_{t-1}^{m}) $$

4. **Weight update**: Compute weights:
    $$\tilde{w}_{t} =  \frac{\color{green}{g}(\mathbf{v}_t \mid \mathbf{s}_{t}^{m})}{\color{green}{g}(\mathbf{v}_{t} \mid \boldsymbol{\mu}_{t_{r}}^{m} )}$$
...
At time $$t=1$$: draw M i.i.d. samples from the "proposal" $$ \color{blue}{f}(\mathbf{s}_1) $$ *and* set $$\lambda_{1}^{m} = 1/M$$
...
$$
\color{#FF8000}{\Psi}_t(\mathbf{s}_t) = \sum_{m=1}^{M} \lambda_{t}^{m} \color{blue}{f}(\mathbf{s}_t \mid \mathbf{s}_{t-1}^{m})
$$
...
$$ \lambda_{t}^{m} = \color{green}{g}(\mathbf{v}_{t} \mid \boldsymbol{\mu}_{t-1}^{m}) \cdot w_{t-1}^{m} $$ 
...
&= \frac{\color{green}{g}(\mathbf{v}_{t} \mid \mathbf{s}_{t}^{m}) \sum_{\color{red}{i}=1}^{M} w_{t-1}^{\color{red}{i}} \color{blue}{f}(\mathbf{s}_{t}^{m} \mid \mathbf{s}_{t-1}^{\color{red}{i}})}{\sum_{\color{red}{i}=1}^{M} \lambda_{t}^{i} \color{blue}{f}(\mathbf{s}_{t}^{m} \mid \mathbf{s}_{t-1}^{\color{red}{i}}} \\
&\approx \frac{\color{green}{g}(\mathbf{v}_{t} \mid \mathbf{s}_{t}^{m}) w_{t-1}^{m}}{\lambda_{t}^{m}}
...
Let's start with the first of the three main stages, namely *proposal adaptation*. In this stage, weights akin to the APF "preweights" are computed in order to build the MIS proposal, which is a mixture (in this case of transition densities or *kernels*, but this is a choice really). It is well known that in MIS, when we draw i.i.d. samples from a mixture, the performance improves when the discrepancy between the proposal and the target density decreases. By performance we mean lowering variance of the importance weights. 
...
\hline
\lambda_{t}^{m} & w_{t}^{m} & \color{green}{g}(\mathbf{v}_{t} \mid \boldsymbol{\mu}_{t-1}^{m}) \cdot w_{t-1}^{m} & \frac{\color{green}{g}(\mathbf{v}_{t} \mid \mathbf{s}_{t}^{m}) \sum_{\color{red}{i}=1}^{M} w_{t-1}^{\color{red}{i}} \color{blue}{f}(\mathbf{s}_{t}^{m} \mid \mathbf{s}_{t-1}^{\color{red}{i}})}{\sum_{\color{red}{i}=1}^{M} \lambda_{t}^{i} \color{blue}{f}(\mathbf{s}_{t}^{m} \mid \mathbf{s}_{t-1}^{\color{red}{i}}} \\
w_{t}^{m} & \color{green}{g}(\mathbf{v}_t \mid \mathbf{s}_{t}^{m}) & \frac{\color{green}{g}(\mathbf{v}_t \mid \mathbf{s}_{t}^{m}) }{} & -8 
\end{array}
$$
