...
So, everything is just computing distributions and applying Bayes rule. An important detail is that we are able to do so efficiently by exploiting the structure of the state space model, i.e. the conditional independencies. 
...
$$
p\left(\mathbf{s}_{t} | \mathbf{v}_{1: t}\right) = \frac{p\left(\mathbf{s}_{t} , \mathbf{v}_{1:t}\right)}{p\left(\mathbf{v}_{1:t} \right)}
$$
